...
public class DatabaseOperations {
	...
	public static List<Utilisateur> getAllUsers() {
		Query queryObj = entityMgrObj.createQuery("SELECT e FROM Utlisateur e");
		List<Utilisateur> cList = queryObj.getResultList();
		if (cList != null && cList.size() > 0) {
			return cList;
		} else {
			return null;
	...
	public static String creerUser(String nomUser, String prenomUser, String emailUser, String adressUser,
			String passwordUser) {
		if (!transactionObj.isActive()) {
			transactionObj.begin();
		}
		
		Utilisateur u = new Utilisateur(0, nomUser, prenomUser, emailUser, adressUser, passwordUser);
	...
	public static Utilisateur getUserById(int idUser) {
		...
		Utilisateur user = entityMgrObj.find(Utilisateur.class, idUser);

		System.out.println("GetUserById : " + user.getId());
		return user;
	}


	public static String updateUser(Utilisateur user) {
		if (!transactionObj.isActive()) {
			transactionObj.begin();
		}

		Query queryObj = entityMgrObj.createQuery(
				"UPDATE Utilisateur u SET u.nomUser=:nom, u.prenomUser=:prenom, u.emailUser=:email, u.adressUser=:adress, u.passwordUser=:password u WHERE u.idUser = "
						+ user.getId());

		queryObj.setParameter("nom", user.getNom());
		queryObj.setParameter("prenom", user.getPrenom());
		queryObj.setParameter("email", user.getEmail());
		queryObj.setParameter("adress", user.getAdresse());
		queryObj.setParameter("password", user.getPassword());
